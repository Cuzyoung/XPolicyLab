...
3. **model.py** — `class Model(ModelTemplate)` implementing `__init__(model_cfg)`, `update_obs`, `update_obs_batch`, `get_action`, `get_action_batch(env_idx_list=None)`, `reset`. Action dimensions come from `get_robot_action_dim_info(env_cfg_type)`, not literals; every supported `env_cfg_type` exists in `utils/robot/_robot_info.json`. `model.py` must not decode images at all — the policy server hands `update_obs` / `update_obs_batch` plain arrays — so flag any decoding there, including `decode_image_bit`. In conversion scripts and training dataloaders that read XPolicyLab data, decoding is required but only via `decode_image_bit` from `XPolicyLab.utils.process_data`; flag hand-rolled `cv2.imdecode` / `np.frombuffer` / PIL decoding (RoboTwin/RoboDojo legacy layouts break custom decoders). Images are RGB end to end: flag any channel swap (`COLOR_BGR2RGB`, `COLOR_RGB2BGR`, `[..., ::-1]`) that is not a medium adapter immediately around `cv2.VideoWriter.write` / `cv2.VideoCapture.read`, and check that the training path and `model.py` apply the same number of swaps.
