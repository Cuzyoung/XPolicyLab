...
for path in "${PYTHON}" "${model_path}/model.safetensors.index.json" "${dataset_path}/meta/info.json" "${norm_stats_path}"; do
  if [[ ! -e "${path}" ]]; then
...
cd "${LINGBOT_ROOT}"
PATH="${VENV_DIR}/bin:${PATH}" bash "${LINGBOT_ROOT}/train.sh" \
...
"${PYTHON}" "${POLICY_DIR}/prepare_bundle.py" \
  ...
  --robot-config "${POLICY_DIR}/robot_configs/yam_dual_absolute.yaml" \
  --native-hz "${native_hz}" \
