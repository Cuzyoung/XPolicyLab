...
class CfgNode(dict):
    ...
    def __init__(self, init_dict=None, key_list=None):
        ...
        for k, v in init_dict.items():
            # Convert dicts nested anywhere in lists / tuples as well as direct
            # child dictionaries. Saved configs can contain list-of-dict values
            # such as dataset_opts.
            v = _convert_to_cfg_node(v, key_list=key_list + [k])
            init_dict[k] = v
        ...
        if "." in key:
            ...
            for subkey in keys[:-1]:
                if subkey not in current:
                    current[subkey] = CfgNode()
                current = current[subkey]
            current[keys[-1]] = value
        else:
            super(CfgNode, self).__setitem__(key, value)
...
def _convert_to_cfg_node(value, key_list):
    if isinstance(value, CfgNode):
        return value
    if type(value) is dict:
        return CfgNode(value, key_list=key_list)
    if isinstance(value, list):
        return [
            _convert_to_cfg_node(item, key_list + [str(i)])
...
def _to_dict(cfg_node):
    ...
    def convert_to_dict(cfg_node, key_list):
        if isinstance(cfg_node, list):
            return [
                convert_to_dict(v, key_list + [str(i)])
                for i, v in enumerate(cfg_node)
            ]
        ...
        if isinstance(cfg_node, dict):
            return {
                k: convert_to_dict(v, key_list + [str(k)])
...
def _decode_cfg_value(v):
    ...
    # will raise a SyntaxError.
        ...
        pass
    return _convert_to_cfg_node(v, key_list=[])
