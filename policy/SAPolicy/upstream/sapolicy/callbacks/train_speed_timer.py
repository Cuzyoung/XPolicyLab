...
class TrainSpeedTimer(pl.Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        if self.last_batch_end is not None:
            ...
            pl_module.log(
                ...
                torch.full((), float(average_time), device=pl_module.device),  # device scalar: a float would be a pageable H2D copy (stream sync)
                on_step=True,
                on_epoch=False,
                prog_bar=False,
                logger=True,
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        pl_module.log(
            ...
            torch.full((), float(average_time), device=pl_module.device),
            on_step=True,
            on_epoch=False,
            prog_bar=False,
            logger=True,
