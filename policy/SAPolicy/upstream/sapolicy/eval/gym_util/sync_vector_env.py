...
class SyncVectorEnv(VectorEnv):
    ...
    def reset(self, seed=None, options=None):
        ...
        if options is None or isinstance(options, dict):
            ...
            if len(options) != self.num_envs:
                raise ValueError("Expected one options dict per environment")
            options_list = list(options)

        self._dones[:] = False
        observations = []
        for env, env_seed, env_opt in zip(self.envs, seeds, options_list):
            kwargs = {}
            if env_seed is not None:
                kwargs['seed'] = env_seed
            if env_opt is not None:
                kwargs['options'] = env_opt
        ...
        try:
            ...
            self._log_concat_error("reset", observations)
            raise
    ...
    def reset_wait(self):
        ...
        try:
            ...
            self._log_concat_error("reset_wait", observations)
            raise
    ...
    def step_wait(self):
        ...
        try:
            ...
            self._log_concat_error("step_wait", observations)
            raise
    ...
    def _log_concat_error(self, stage, observations):
        if not observations:
            logger.error(f"[SyncVectorEnv.{stage}] No observations to concatenate.")
            return
        ...
        if isinstance(first_obs, dict):
            ...
            obs_info = getattr(first_obs, "shape", None)
        logger.error(
            "[SyncVectorEnv.%s] concatenate failed. Expected: %s, observed: %s",
            stage,
            self._space_shapes(self.single_observation_space),
            obs_info,
        )

    def _space_shapes(self, space):
        if hasattr(space, "spaces"):
            return {k: getattr(v, "shape", None) for k, v in space.spaces.items()}
        return getattr(space, "shape", None)
