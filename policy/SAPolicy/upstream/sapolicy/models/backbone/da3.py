...
https://github.com/cvlab-kaist/GLD/blob/main/src/stage1/encoders/da3.py
...
class DA3Encoder(nn.Module):
    ...
    def __init__(
        ...
        if os.path.isfile(pretrained_path):
            # Local safetensors/pth file: init architecture from HF, then load local weights
            da3 = DepthAnything3.from_pretrained("depth-anything/DA3-Base")
            if pretrained_path.endswith('.safetensors'):
                from safetensors.torch import load_file
                state = load_file(pretrained_path)
            else:
                state = torch.load(pretrained_path, map_location='cpu')
            da3.load_state_dict(state, strict=False)
        else:
            # Local snapshot dirs must not hit the Hub (offline training pods).
            kwargs = {}
            if os.path.isdir(pretrained_path):
                kwargs["local_files_only"] = True
            da3 = DepthAnything3.from_pretrained(pretrained_path, **kwargs)
...
# Backward-compatible name used by sapolicy and configs
DA3Backbone = DA3Encoder