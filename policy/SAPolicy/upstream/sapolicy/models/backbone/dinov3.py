"""
DINOv3 Vision Transformer wrapper for SpatialAlignVLA

Based on the official DINOv3 implementation:
https://github.com/facebookresearch/dinov3

DINOv3 improves upon DINOv2 with:
- Better training procedure
- Improved vision features
- Enhanced zero-shot transfer capabilities
"""
...
import logging
from typing import Callable, Any, Dict, List, Literal, Optional, Sequence, Tuple, Union

# Reuse DINOv2 architecture components
...
def DINOv3(model_name, img_size=518, use_registers=True):
    """
    ...
    if model_name not in model_zoo:
        raise ValueError(f"Unknown model: {model_name}. Choose from {list(model_zoo.keys())}")

    n_storage_tokens = 4 if use_registers else 0

    logger.info(f"Creating DINOv3 {model_name} model with {n_storage_tokens} storage tokens")
    ...
    model = model_zoo[model_name](
        ...
        ffn_layer="mlp" if model_name not in ("vitg",) else "swiglu128",
...
def load_dinov3_pretrained(model, model_name, device='cpu'):
    """
    Load pretrained DINOv3 weights from torch hub

    Args:
        model: DINOv3 model instance
        model_name: One of ['vits', 'vitb', 'vitl', 'vitg']
    ...
    try:
        ...
        # Note: This requires the official dinov3 weights to be published
        logger.info(f"Attempting to load DINOv3 {model_name} pretrained weights from torch hub")

        # Map model names to torch hub identifiers
        hub_names = {
            'vits': 'dinov3_vits16',
            'vitb': 'dinov3_vitb16',
            'vitl': 'dinov3_vitl16',
        ...
        if model_name in hub_names:
            try:
                pretrained = torch.hub.load('facebookresearch/dinov3', hub_names[model_name])
                model.load_state_dict(pretrained.state_dict(), strict=False)
                logger.info(f"Successfully loaded DINOv3 {model_name} pretrained weights")
