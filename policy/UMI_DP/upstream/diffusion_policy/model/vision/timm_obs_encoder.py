...
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.model.common.module_attr_mixin import ModuleAttrMixin

from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.common.pytorch_util import replace_submodules
...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
            ...
            # normalize RGB using the backbone's pretrained mean/std
        ...
        # Resolve before a CNN backbone is replaced by a Sequential below.
        # Keep this in the encoder so training and predict_action share it,
        # including inference construction with pretrained=False.
        self.rgb_normalize = nn.Identity()
        if imagenet_norm:
            self.rgb_normalize = torchvision.transforms.Normalize(
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                image_shape = shape[1:]
        if transforms and not isinstance(transforms[0], torch.nn.Module):
        ...
        if imagenet_norm and any(
                isinstance(module, torchvision.transforms.Normalize)
                for module in transform.modules()):
            raise ValueError(
                'imagenet_norm=True already normalizes RGB; remove Normalize from transforms')
