...
import numpy as np
import shutil
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.workspace.base_workspace import BaseWorkspace
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.policy.diffusion_unet_image_policy import DiffusionUnetImagePolicy
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.dataset.base_dataset import BaseImageDataset, BaseDataset
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.env_runner.base_image_runner import BaseImageRunner
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.common.checkpoint_util import TopKCheckpointManager
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.common.json_logger import JsonLogger
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.common.pytorch_util import dict_apply, optimizer_to
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.model.diffusion.ema_model import EMAModel
from XPolicyLab.policy.UMI_DP.upstream.diffusion_policy.model.common.lr_scheduler import get_scheduler
...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        env_runner = hydra.utils.instantiate(
            cfg.task.env_runner,
            output_dir=self.output_dir) if cfg.task.env_runner is not None else None
        assert env_runner is None or isinstance(env_runner, BaseImageRunner)
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                # checkpoint
                # Epoch indices are zero-based. Save after every completed
                # checkpoint interval (e.g. 4, 9, 14, 19 for an interval of 5),
                # rather than saving prematurely at epoch 0.
